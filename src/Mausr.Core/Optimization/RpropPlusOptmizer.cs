...
namespace Mausr.Core.Optimization {
	public class RpropPlusOptmizer : IGradientBasedOptimizer {
		...
		public RpropPlusOptmizer(double initialStep, double maxStep, double stepUpMult, double stepDownMult, int maxIters) {
		...
		public bool Optimize(Vector<double> resultAndInitPos, IFunctionWithDerivative function, double minDerivCompMaxMagn,
			...
			var stepWeight = new DenseVector(function.DimensionsCount);
			...
			for (int iter = 0; iter < maxIters; ++iter) {
				...
				for (int i = 0; i < prevStepSigns.Length; ++i) {
					...
					if (sign > 0) {
						...
						stepWeight[i] = -currSign * stepSize[i];
					}
					else if (sign < 0) {
						stepSize[i] = Math.Max(minStep, stepSize[i] * stepDownMult);
						stepWeight[i] = -stepWeight[i];
						currSign = 0;
					}
					else {
						// Step size do not change.
						stepWeight[i] = -currSign * stepSize[i];
					}
					
					resultAndInitPos[i] += stepWeight[i];
